...
CoinbaseGridTrader = agent.CoinbaseGridTrader
LiveRiskError = agent.LiveRiskError


def _build_trader(tmp_path, monkeypatch) -> CoinbaseGridTrader:
    monkeypatch.chdir(tmp_path)
    trader = CoinbaseGridTrader.__new__(CoinbaseGridTrader)
    trader.config = {
...
def test_quote_reserve_skips_buy_orders(tmp_path, monkeypatch) -> None:
    ...
    trader._place_order = lambda product_id, side, price, size: placed.append((side, price, size))

    trader._place_grid_orders(
        {"buy": [{"price": 95.0, "size": 10.0}, {"price": 10.0, "size": 1.0}], "sell": []},
        set(),
        "BTC-USD",
        1000.0,
        {},
...
def test_drawdown_guard_persists_peak_and_blocks(tmp_path, monkeypatch) -> None:
    trader = _build_trader(tmp_path, monkeypatch)
    first = trader._enforce_live_risk(reference_price=100.0, base_balance=1.0, quote_balance=100.0)
    assert first["peak_equity_usd"] == 200.0

    trader.live_risk_state = {"peak_equity_usd": 200.0}
    with pytest.raises(LiveRiskError):
        trader._enforce_live_risk(reference_price=100.0, base_balance=0.0, quote_balance=150.0)
