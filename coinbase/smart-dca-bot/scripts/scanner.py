...
class OpportunityScanner:
    ...
    def __init__(
        ...
        *,
        ...
        require_coinbase_verified: bool = False,
    ) -> None:
        ...
        self.require_coinbase_verified = bool(require_coinbase_verified)
    ...
    def scan(
        ...
        base_anchor = max(base_allocations.items(), key=lambda item: item[1])[0] if base_allocations else "BTC-USD"
        ...
        for row in market_rows:
            ...
            market_cap = float(row.get("market_cap_usd", 0.0))
            if market_cap < self.min_market_cap_usd:
                continue

            coinbase_verified = bool(row.get("coinbase_verified", True))
            if self.require_coinbase_verified and not coinbase_verified:
                continue
            ...
            learn_reward_usd = float(row.get("learn_reward_usd", 0.0))
            ...
            if "learn_earn" in self.enabled_signals and learn_reward_usd > 0:
                realloc = self._cap(5.0 + min(learn_reward_usd, 20.0) * 0.5)
                signals.append(
                    ScannerSignal(
                        signal_id=self._signal_id("learn_earn", asset),
                        signal_type="learn_earn",
                        asset=asset,
                        confidence_pct=round(min(55.0 + learn_reward_usd * 2.0, 82.0), 2),
                        trigger_data={"learn_reward_usd": learn_reward_usd},
                        suggestion=(
                            f"Coinbase Learn reward detected. Shift up to {realloc:.2f}% into {asset} and claim rewards first."
                        ),
                        reallocation_pct=realloc,
                    )
                )
