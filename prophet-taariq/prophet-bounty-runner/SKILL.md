...
3. Run `PROPHET_SESSION_TOKEN=$JWT python3 scripts/agent.py --command run --json-output` once and confirm the result is
...
4. Only after a successful (or explicitly acknowledged) first run,
   ...
   autonomous 6h runs and start `python3 scripts/run_local_pull_runner.py` to claim due ticks.
...
9. **Export and shell out.**
     `PROPHET_SESSION_TOKEN=<jwt> python3 scripts/agent.py --command run --json-output`. Surface `status` to the
...
- Run `python3 scripts/setup_cron.py list` to find the pause
  ...
  - `auto_pause_reason=low_serenbucks`: top up at
    `https://serendb.com/serenbucks`, then `setup_cron.py resume`.
