...
_REMEDIATION_NO_APPROVALS = (
    ...
    "Polymarket exchange contracts. On the next live cycle "
    "(`live_mode=true` + `--yes-live`), the bot will broadcast "
    "approve()/setApprovalForAll() to the pinned Polymarket spenders "
    "(CTF Exchange, NegRisk CTF Exchange, NegRisk Adapter) "
    "automatically — no new deposit is needed and no manual action is "
    "required. For dry-run cycles, or to grant allowances ahead of "
    "going live, visit https://polymarket.com/wallet."
)
