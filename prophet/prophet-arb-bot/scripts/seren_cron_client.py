...
# Inherited from prophet-bounty-runner (#469): 12h polling matches a
# trade-tick cadence wide enough to keep poll cost negligible while still
# recovering one missed tick before the next scheduled run. The arb-bot
# fires every hour; the runner can still ride a server-issued
# `next_poll_seconds` override if Seren asks for tighter polling.
DEFAULT_POLL_INTERVAL_SECONDS = 12 * 60 * 60  # 12 hours
DEFAULT_TIMEOUT_SECONDS = 30.0

# Arb cadence: every hour, on the hour, UTC. Faster than the bounty
# runner's 6h because Prophet ↔ Polymarket drift is a sub-day signal.
