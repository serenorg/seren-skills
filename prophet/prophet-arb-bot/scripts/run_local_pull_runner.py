#!/usr/bin/env python3
"""Local-pull runner for prophet-arb-bot.

Long-lived process that polls seren-cron, claims due jobs, and executes
`agent.py --command run --json-output` with the saved local payload.
After each subprocess exit, inspects the output for auto-pause signals
(low SerenBucks 402) and pauses the cron job before submitting the
per-tick result.
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="prophet-arb-bot local-pull runner.")
...
def build_agent_command(local_payload: dict[str, Any], default_config: str) -> list[str]:
    ...
    The arb-bot's agent.py reads prophet_email/email_provider from
    config.inputs, so the runner only forwards --config / --command /
    --json-output / --yes-live (when the operator scheduled a live tick).
    """
    cmd = [
        sys.executable,
    ...
    if local_payload.get("yes_live") and not local_payload.get("dry_run"):
        cmd.append("--yes-live")
    return cmd
