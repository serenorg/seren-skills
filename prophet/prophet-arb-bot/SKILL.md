...
- **`phase=cold_launch` marker.** When the TimeoutError carries `phase=cold_launch`, the bundled MCP child was mid-`chromium.launch()` on the first `tools/call` after the MCP `initialize` handshake. Diagnostic-only since #652 — the per-call gateway ceiling is now 180s and the per-entry budget on the `/create` driver is the real cap. The marker just lets operators tell at a glance whether budget was eaten by the cold Chromium launch or by later steady-state stages. Common cold-launch culprits: a long-running user Chrome instance forcing Gatekeeper re-validation, a slow disk on `~/Library/Caches/ms-playwright`, or a missing Chrome install.

**`create_market_via_ui_entry_budget_exceeded` (#652).**

- The `/create` UI driver overran its per-entry wall-clock budget (`auto_discover.create_market_entry_budget_seconds`, default 300s). The blocked envelope carries `entry_budget_seconds`, `entry_elapsed_seconds`, and `entry_last_stage` so the operator can see which stage was in flight when the budget tripped.
- If the budget tripped after the Polymarket hedge had committed, the blocked envelope also carries `unwind_status` — the runner invokes `record-created-market` with `prophet_confirm_declined=True` automatically before returning, so the Polymarket leg is reversed and no naked exposure is left behind. Confirm the unwind status before continuing.
- The cycle continues to the next pending entry; one budget-exceeded entry does not poison the rest. If the same entry hits the budget two cycles in a row, either raise `create_market_entry_budget_seconds` for a slow host or investigate the stage named in `entry_last_stage` (most often a Playwright navigation under heavy peer-Chrome contention — see the #647 troubleshooting entry above).
