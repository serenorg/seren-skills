...
- **`phase=cold_launch` marker (#649).** When the TimeoutError carries `phase=cold_launch`, the bundled MCP child is mid-`chromium.launch()` on the first `tools/call` after the MCP `initialize` handshake. The gateway already grants 120s for this first call (vs the 30s steady-state); a `phase=cold_launch` timeout means the host's cold Chromium launch genuinely exceeds 2 minutes. Usual culprits: a long-running user Chrome instance forcing Gatekeeper re-validation, a slow disk on `~/Library/Caches/ms-playwright`, or a missing Chrome install. Without `phase=cold_launch` the timeout is steady-state and the cause is elsewhere (network call from the page under automation, etc.).
