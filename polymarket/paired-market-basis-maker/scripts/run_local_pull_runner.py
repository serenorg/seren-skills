#!/usr/bin/env python3
"""Poll seren-cron local pull jobs for paired-market-basis-maker."""
...
SKILL_SLUG = "paired-market-basis-maker"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the paired-market-basis-maker seren-cron local pull runner."
    )
    ...
    parser.add_argument("--machine-label", default=current_machine_label(), help="Friendly runner machine label.")
    parser.add_argument(
        "--poll-interval-seconds",
        type=int,
        default=DEFAULT_SEREN_CRON_POLL_INTERVAL_SECONDS,
        help="Fallback poll cadence when seren-cron does not provide next_poll_seconds.",
    )
    parser.add_argument("--once", action="store_true", help="Poll once, execute one job if available, then exit.")
...
def _build_command(local_payload: dict[str, Any], default_config: str) -> list[str]:
    command = [
        ...
        "--run-type",
        safe_str(local_payload.get("run_type"), DEFAULT_RUN_TYPE) or DEFAULT_RUN_TYPE,
    ]
    markets_file = safe_str(local_payload.get("markets_file"), "")
    backtest_file = safe_str(local_payload.get("backtest_file"), "")
    if markets_file:
        command.extend(["--markets-file", markets_file])
    if backtest_file:
        command.extend(["--backtest-file", backtest_file])
    backtest_days = local_payload.get("backtest_days")
    if backtest_days is not None:
        command.extend(["--backtest-days", str(safe_int(backtest_days, 0))])
    if local_payload.get("allow_negative_backtest"):
        command.append("--allow-negative-backtest")
...
def main() -> int:
    ...
    try:
        ...
        while True:
            ...
            if safe_str(poll_result.get("action"), "") != "run":
                if args.once:
                    print(json.dumps({"status": "ok", "runner_id": runner_id, "action": poll_result.get("action", "idle")}, sort_keys=True))
                    return 0
                time.sleep(max(1, safe_int(poll_result.get("next_poll_seconds"), args.poll_interval_seconds)))
                continue
            ...
            execution_result_id = safe_str(execution_result.get("id"), "")
