---
name: family-council-cycle-operator
description: "Family office: Drafts agendas, ingests minutes into commitments, nudges owners, and folds trustee retirement, document refresh, and beneficiary age tripwires into the register."
---

# Family Office · Family Council Cycle Operator

## For Claude: How to Use This Skill

Skill instructions are preloaded in context when this skill is active. Do not perform filesystem searches or tool-driven exploration to rediscover them; use the guidance below directly.

This is a recurring family-office operator skill, not a memo generator. Run it to operationalize `family-meeting-agenda-minutes-template and knowledge` as a stateful, approval-gated employee.

## When to Use

- run family council cycle
- draft family meeting agenda
- track governance commitments
- ingest family council minutes

## Operating Contract

This operator drafts agendas, ingests minutes into commitments, nudges owners, and folds trustee retirement, document refresh, and beneficiary age tripwires into the register. It belongs to `Family Governance, Succession & Next-Gen` in the Citi family-office functions frame and is priority `P1` from roadmap issue #852.

Approval gate: official minutes publication and owner nudges are review-gated. The agent stages review packets, audit rows, reminders, and draft communications. It does not submit money movement, government filings, binding notices, beneficiary changes, or external instructions without the live gate and human approval.

## Schema Guard

Before any read or write, resolve or create the SerenDB project `family-office-family-council-cycle-operator` and database `family_office_family_council_cycle_operator`. Verify or create these tables:
...
- `draft_agenda`
- `ingest_commitments`
- `prepare_owner_nudges`

## Cloud Deployment

Deploy as a seren-cloud cron or event-triggered worker with the default Python runtime. Suggested cadence: `meeting-event-t5-agenda-post-meeting-ingest-daily-commitment-watch`. Do not use Docker, a local pull runner, or customer-specific local paths.
...
```bash
python3 -m pytest family-office/family-council-cycle-operator/tests -q
python3 family-office/family-council-cycle-operator/scripts/agent.py --functional-test --config family-office/family-council-cycle-operator/config.example.json
