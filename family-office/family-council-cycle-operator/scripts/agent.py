#!/usr/bin/env python3
"""Dry-run-first operator runtime for family-council-cycle-operator."""
...
SKILL_NAME = "family-council-cycle-operator"
PILLAR = "legacy-preservation"
DEFAULT_DRY_RUN_TO = "taariq@serendb.com"
STATE_TABLES = ["commitment_register", "meeting_log", "agenda_items", "owner_nudges"]
...
def _profile(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    profile.setdefault("title", "Family Council & Commitment-to-Close Operator")
    profile.setdefault("priority", "P1")
    profile.setdefault("cadence", "meeting-event-t5-agenda-post-meeting-ingest-daily-commitment-watch")
    profile.setdefault("gate", "official minutes publication and owner nudges are review-gated")
    profile.setdefault("sample_subject", "June family council")
    profile.setdefault("exception_flags", ["commitment_overdue"])
    profile.setdefault("actions", ["draft_agenda", "ingest_commitments", "prepare_owner_nudges"])
...
def _schema_guard() -> Dict[str, Any]:
    return {
        "database": "family_office_family_council_cycle_operator",
        "tables": STATE_TABLES,
