---
name: ap-invoice-intake-and-bill-pay-operator
description: "Family office: Mines ap inboxes, parses invoices, dedupes, matches vendors and recurring contracts, codes to the chart of accounts, and stages review-gated bill-pay runs."
---

# Family Office · Ap Invoice Intake And Bill Pay Operator
...
- run accounts payable invoice intake
- stage bill pay run for review
- check invoice duplicates and vendor bank changes
- triage family office AP inbox

## Operating Contract

This operator mines AP inboxes, parses invoices, dedupes, matches vendors and recurring contracts, codes to the chart of accounts, and stages review-gated bill-pay runs. It belongs to `Reporting, Accounting, Cash Management & Bill-Pay` in the Citi family-office functions frame and is priority `P0` from roadmap issue #852.

Approval gate: payments never release without human approval and beneficiary verification for remit changes. The agent stages review packets, audit rows, reminders, and draft communications. It does not submit money movement, government filings, binding notices, beneficiary changes, or external instructions without the live gate and human approval.

## Schema Guard

Before any read or write, resolve or create the SerenDB project `family-office-ap-invoice-intake-and-bill-pay-operator` and database `family_office_ap_invoice_intake_and_bill_pay_operator`. Verify or create these tables:
...
- `parse_invoice`
- `stage_payment_run`
- `open_remit_change_verification`

## Cloud Deployment

Deploy as a seren-cloud cron or event-triggered worker with the default Python runtime. Suggested cadence: `daily-inbox-sweep-twice-weekly-payment-run`. Do not use Docker, a local pull runner, or customer-specific local paths.
...
```bash
python3 -m pytest family-office/ap-invoice-intake-and-bill-pay-operator/tests -q
python3 family-office/ap-invoice-intake-and-bill-pay-operator/scripts/agent.py --functional-test --config family-office/ap-invoice-intake-and-bill-pay-operator/config.example.json
