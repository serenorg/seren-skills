#!/usr/bin/env python3
"""Dry-run-first operator runtime for ap-invoice-intake-and-bill-pay-operator."""
...
SKILL_NAME = "ap-invoice-intake-and-bill-pay-operator"
PILLAR = "complexity-management"
DEFAULT_DRY_RUN_TO = "taariq@serendb.com"
STATE_TABLES = ["invoice_register", "vendor_master", "payment_runs", "duplicate_hashes", "approval_tasks"]
...
def _profile(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    profile.setdefault("title", "Accounts-Payable Invoice Intake & Review-Gated Bill-Pay Operator")
    profile.setdefault("priority", "P0")
    profile.setdefault("cadence", "daily-inbox-sweep-twice-weekly-payment-run")
    profile.setdefault("gate", "payments never release without human approval and beneficiary verification for remit changes")
    profile.setdefault("sample_subject", "Acme Services Invoice 1042")
    profile.setdefault("exception_flags", ["remit_to_changed"])
    profile.setdefault("actions", ["parse_invoice", "stage_payment_run", "open_remit_change_verification"])
...
def _schema_guard() -> Dict[str, Any]:
    return {
        "database": "family_office_ap_invoice_intake_and_bill_pay_operator",
        "tables": STATE_TABLES,
