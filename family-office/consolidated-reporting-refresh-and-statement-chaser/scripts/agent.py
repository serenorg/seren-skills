#!/usr/bin/env python3
"""Dry-run-first operator runtime for consolidated-reporting-refresh-and-statement-chaser."""
...
SKILL_NAME = "consolidated-reporting-refresh-and-statement-chaser"
PILLAR = "complexity-management"
DEFAULT_DRY_RUN_TO = "taariq@serendb.com"
STATE_TABLES = ["report_versions", "expected_documents", "distribution_log", "statement_chase_tasks"]
...
def _profile(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    profile.setdefault("title", "Consolidated Reporting Refresh + K-1 / Statement Collection Chaser")
    profile.setdefault("priority", "P1")
    profile.setdefault("cadence", "report-cadence-refresh-weekly-document-chase-tax-season-intensified")
    profile.setdefault("gate", "external report distribution and outbound chasers are review-gated")
    profile.setdefault("sample_subject", "June consolidated report")
    profile.setdefault("exception_flags", ["missing_statement"])
    profile.setdefault("actions", ["refresh_report_snapshot", "open_statement_chase", "stage_distribution_review"])
...
def _schema_guard() -> Dict[str, Any]:
    return {
        "database": "family_office_consolidated_reporting_refresh_and_statement_chaser",
        "tables": STATE_TABLES,
