#!/usr/bin/env python3
"""Dry-run-first operator runtime for household-payroll-cycle-operator."""
...
SKILL_NAME = "household-payroll-cycle-operator"
PILLAR = "complexity-management"
DEFAULT_DRY_RUN_TO = "taariq@serendb.com"
STATE_TABLES = ["staff_register", "payroll_ledger", "tax_deposit_schedule", "timesheet_exceptions"]
...
def _profile(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    profile.setdefault("title", "Household Staff Payroll & Compliance Cycle Operator")
    profile.setdefault("priority", "P0")
    profile.setdefault("cadence", "per-pay-period-daily-document-expiry-quarterly-employer-tax-package")
    profile.setdefault("gate", "ACH releases, tax remittances, and filings require dual gate approval")
    profile.setdefault("sample_subject", "June household payroll")
    profile.setdefault("exception_flags", ["timesheet_missing_approval"])
    profile.setdefault("actions", ["validate_timesheets", "stage_pay_run", "open_payroll_exception"])
...
def _schema_guard() -> Dict[str, Any]:
    return {
        "database": "family_office_household_payroll_cycle_operator",
        "tables": STATE_TABLES,
