#!/usr/bin/env python3
"""Dry-run-first operator runtime for governance-document-version-control."""
...
SKILL_NAME = "governance-document-version-control"
PILLAR = "legacy-preservation"
DEFAULT_DRY_RUN_TO = "taariq@serendb.com"
STATE_TABLES = ["document_versions", "signatories", "review_windows", "drift_flags"]
...
def _profile(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    profile.setdefault("title", "Governance Document Version Control & Drift Watch")
    profile.setdefault("priority", "P1")
    profile.setdefault("cadence", "daily-document-hash-watch-weekly-review-window-digest")
    profile.setdefault("gate", "promoting a document to ratified requires explicit human confirmation")
    profile.setdefault("sample_subject", "Investment policy draft")
    profile.setdefault("exception_flags", ["unratified_change_detected"])
    profile.setdefault("actions", ["hash_governance_documents", "open_signature_task", "flag_policy_drift"])
...
def _schema_guard() -> Dict[str, Any]:
    return {
        "database": "family_office_governance_document_version_control",
        "tables": STATE_TABLES,
