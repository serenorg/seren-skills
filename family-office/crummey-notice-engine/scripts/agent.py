#!/usr/bin/env python3
"""Dry-run-first operator runtime for crummey-notice-engine."""
...
SKILL_NAME = "crummey-notice-engine"
PILLAR = "legacy-preservation"
DEFAULT_DRY_RUN_TO = "taariq@serendb.com"
STATE_TABLES = ["crummey_register", "notice_ledger", "annual_gift_tracker", "acknowledgment_log"]
...
def _profile(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    profile.setdefault("title", "Crummey Notice & Gift-Funding Deadline Engine")
    profile.setdefault("priority", "P1")
    profile.setdefault("cadence", "on-gift-event-daily-countdown-scheduled-premium-window")
    profile.setdefault("gate", "notices are drafted and sent only after review")
    profile.setdefault("sample_subject", "ILIT premium contribution")
    profile.setdefault("exception_flags", ["notice_window_open"])
    profile.setdefault("actions", ["draft_beneficiary_notices", "start_withdrawal_window_countdown", "log_notice_ledger"])
...
def _schema_guard() -> Dict[str, Any]:
    return {
        "database": "family_office_crummey_notice_engine",
        "tables": STATE_TABLES,
