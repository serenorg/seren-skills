#!/usr/bin/env python3
"""Dry-run-first operator runtime for esg-impact-position-tagging-engine."""
...
SKILL_NAME = 'esg-impact-position-tagging-engine'
ISSUE_NUMBER = 948
PILLAR = 'investment-operations'
DEFAULT_DRY_RUN_TO = 'taariq@serendb.com'
PROJECT_NAME = 'family-office-esg-impact-position-tagging-engine'
DATABASE_NAME = 'family_office_esg_impact_position_tagging_engine'
STATE_TABLES = ['position_tags', 'mandate_rules', 'third_party_esg_sources', 'impact_rollups', 'exclusion_breaches', 'quarterly_report_drafts', 'source_citations']
REQUIRED_OUTPUTS = ['tagged_positions', 'rollup_scores', 'quarterly_impact_report', 'exclusion_breach_list', 'source_citations']
HANDOFFS = ['esg-impact-investing-mandate', 'manager-dd-esg-impact']
AVAILABLE_CONNECTORS = ['docreader', 'docs', 'drive', 'exa', 'outlook', 'passwords', 'search', 'sec', 'sheets', 'storage']
DEFAULT_PROFILE = {'title': 'ESG Impact Position Tagging Engine Operator', 'priority': 'P2', 'cadence': 'monthly-position-tag-refresh-quarterly-impact-report', 'gate': 'investment action, mandate waiver, and external impact-report distribution require human approval', 'sample_subject': 'Public equities and fund look-through ESG mandate review', 'exception_flags': ['exclusion_breach_detected', 'missing_data_source'], 'actions': ['tag_position_sdg_emissions_controversy', 'merge_third_party_data', 'compare_to_mandate', 'draft_quarterly_report'], 'outputs': ['tagged_positions', 'rollup_scores', 'quarterly_impact_report', 'exclusion_breach_list', 'source_citations'], 'handoffs': ['esg-impact-investing-mandate', 'manager-dd-esg-impact'], 'always_review': True}
