#!/usr/bin/env python3
"""Dry-run-first operator runtime for foundation-payout-compliance-monitor."""
...
SKILL_NAME = "foundation-payout-compliance-monitor"
PILLAR = "legacy-preservation"
DEFAULT_DRY_RUN_TO = "taariq@serendb.com"
STATE_TABLES = ["foundation_compliance_years", "asset_value_history", "qualifying_distributions", "payout_gap_alerts"]
...
def _profile(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    profile.setdefault("title", "Foundation 5% Payout & 990-PF Compliance Monitor")
    profile.setdefault("priority", "P0")
    profile.setdefault("cadence", "monthly-recompute-weekly-final-quarter-annual-990pf-rollup")
    profile.setdefault("gate", "monitor only; CPA and board approval are required before filings or final figures")
    profile.setdefault("sample_subject", "2026 foundation payout gap")
    profile.setdefault("exception_flags", ["payout_gap_projected"])
    profile.setdefault("actions", ["recompute_payout_requirement", "project_qualifying_distribution_gap", "draft_board_memo"])
...
def _schema_guard() -> Dict[str, Any]:
    return {
        "database": "family_office_foundation_payout_compliance_monitor",
        "tables": STATE_TABLES,
