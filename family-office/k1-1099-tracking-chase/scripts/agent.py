#!/usr/bin/env python3
"""Dry-run-first operator runtime for k1-1099-tracking-chase."""
...
SKILL_NAME = "k1-1099-tracking-chase"
PILLAR = "complexity-management"
DEFAULT_DRY_RUN_TO = "taariq@serendb.com"
STATE_TABLES = ["expected_documents", "receipt_match_log", "chase_tasks", "cpa_package_log"]
...
def _profile(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    profile.setdefault("title", "K-1 / 1099 Receipt Tracking & Chase")
    profile.setdefault("priority", "P0")
    profile.setdefault("cadence", "daily-tax-season-mining-weekly-chase-cycle")
    profile.setdefault("gate", "chase emails and CPA package pushes are review-gated")
    profile.setdefault("sample_subject", "Acme Fund 2025 K-1")
    profile.setdefault("exception_flags", ["document_overdue"])
    profile.setdefault("actions", ["match_document_receipts", "open_chase_task", "draft_cpa_package_delta"])
...
def _schema_guard() -> Dict[str, Any]:
    return {
        "database": "family_office_k1_1099_tracking_chase",
        "tables": STATE_TABLES,
