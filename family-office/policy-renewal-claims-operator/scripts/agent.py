#!/usr/bin/env python3
"""Dry-run-first operator runtime for policy-renewal-claims-operator."""
...
SKILL_NAME = "policy-renewal-claims-operator"
PILLAR = "complexity-management"
DEFAULT_DRY_RUN_TO = "taariq@serendb.com"
STATE_TABLES = ["policy_register", "renewal_windows", "coverage_gaps", "claims_ledger"]
...
def _profile(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    profile.setdefault("title", "Insurance Policy Renewal, Coverage-Gap & Claims Operator")
    profile.setdefault("priority", "P0")
    profile.setdefault("cadence", "daily-renewal-window-scan-weekly-coverage-gap-digest")
    profile.setdefault("gate", "binding coverage changes and claim submissions are review-gated")
    profile.setdefault("sample_subject", "Umbrella renewal")
    profile.setdefault("exception_flags", ["coverage_gap_detected"])
    profile.setdefault("actions", ["scan_renewal_windows", "flag_coverage_gap", "draft_broker_packet"])
...
def _schema_guard() -> Dict[str, Any]:
    return {
        "database": "family_office_policy_renewal_claims_operator",
        "tables": STATE_TABLES,
