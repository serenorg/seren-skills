#!/usr/bin/env python3
"""Dry-run-first operator runtime for vendor-sla-renewal-kyc-monitor."""
...
SKILL_NAME = "vendor-sla-renewal-kyc-monitor"
PILLAR = "complexity-management"
DEFAULT_DRY_RUN_TO = "taariq@serendb.com"
STATE_TABLES = ["vendor_register", "screening_history", "incident_log", "renewal_tasks", "coi_tracker"]
...
def _profile(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    profile.setdefault("title", "Vendor & Advisor Roster SLA / Renewal / KYC Monitor")
    profile.setdefault("priority", "P1")
    profile.setdefault("cadence", "daily-expiry-notice-window-weekly-vendor-health-monthly-quarterly-screening")
    profile.setdefault("gate", "outbound notices, terminations, and onboarding state changes require approval")
    profile.setdefault("sample_subject", "Security vendor renewal")
    profile.setdefault("exception_flags", ["notice_window_open"])
    profile.setdefault("actions", ["scan_notice_windows", "run_vendor_rescreen", "draft_renewal_packet"])
...
def _schema_guard() -> Dict[str, Any]:
    return {
        "database": "family_office_vendor_sla_renewal_kyc_monitor",
        "tables": STATE_TABLES,
