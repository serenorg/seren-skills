---
name: vendor-sla-renewal-kyc-monitor
description: "Family office: Tracks vendor contracts, notice windows, cois, overdue advisor reviews, sla incidents, kyc and sanctions re-screening, and renewal or termination drafts."
---

# Family Office · Vendor Sla Renewal Kyc Monitor

## For Claude: How to Use This Skill

Skill instructions are preloaded in context when this skill is active. Do not perform filesystem searches or tool-driven exploration to rediscover them; use the guidance below directly.

This is a recurring family-office operator skill, not a memo generator. Run it to operationalize `external-advisor-management-plan` as a stateful, approval-gated employee.

## When to Use

- monitor vendor renewals
- check advisor KYC
- track COI expirations
- run vendor SLA health digest

## Operating Contract

This operator tracks vendor contracts, notice windows, COIs, overdue advisor reviews, SLA incidents, KYC and sanctions re-screening, and renewal or termination drafts. It belongs to `Lifestyle, Concierge & Family-Office Administration` in the Citi family-office functions frame and is priority `P1` from roadmap issue #852.

Approval gate: outbound notices, terminations, and onboarding state changes require approval. The agent stages review packets, audit rows, reminders, and draft communications. It does not submit money movement, government filings, binding notices, beneficiary changes, or external instructions without the live gate and human approval.

## Schema Guard

Before any read or write, resolve or create the SerenDB project `family-office-vendor-sla-renewal-kyc-monitor` and database `family_office_vendor_sla_renewal_kyc_monitor`. Verify or create these tables:
...
- `scan_notice_windows`
- `run_vendor_rescreen`
- `draft_renewal_packet`

## Cloud Deployment

Deploy as a seren-cloud cron or event-triggered worker with the default Python runtime. Suggested cadence: `daily-expiry-notice-window-weekly-vendor-health-monthly-quarterly-screening`. Do not use Docker, a local pull runner, or customer-specific local paths.
...
```bash
python3 -m pytest family-office/vendor-sla-renewal-kyc-monitor/tests -q
python3 family-office/vendor-sla-renewal-kyc-monitor/scripts/agent.py --functional-test --config family-office/vendor-sla-renewal-kyc-monitor/config.example.json
