#!/usr/bin/env python3
"""Dry-run-first operator runtime for multi-entity-reconciliation-gl-close-and-fee-audit-operator."""
...
SKILL_NAME = "multi-entity-reconciliation-gl-close-and-fee-audit-operator"
PILLAR = "complexity-management"
DEFAULT_DRY_RUN_TO = "taariq@serendb.com"
STATE_TABLES = ["reconciliation_runs", "open_items", "close_periods", "fee_recompute_audit", "intercompany_journals"]
...
def _profile(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    profile.setdefault("title", "Bank/Custodian Reconciliation, Multi-Entity GL Close & Fund Fee/NAV Audit Operator")
    profile.setdefault("priority", "P0")
    profile.setdefault("cadence", "daily-transaction-pull-monthly-close-quarterly-fee-true-up")
    profile.setdefault("gate", "non-routine entries, material reclasses, and fee dispute sends require approval")
    profile.setdefault("sample_subject", "May close exception")
    profile.setdefault("exception_flags", ["unmatched_custodian_cash"])
    profile.setdefault("actions", ["match_transactions", "score_close_readiness", "draft_fee_exception_packet"])
...
def _schema_guard() -> Dict[str, Any]:
    return {
        "database": "family_office_multi_entity_reconciliation_gl_close_and_fee_audit_operator",
        "tables": STATE_TABLES,
