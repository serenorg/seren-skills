#!/usr/bin/env python3
"""Dry-run-first operator runtime for property-asset-maintenance-scheduler."""
...
SKILL_NAME = "property-asset-maintenance-scheduler"
PILLAR = "complexity-management"
DEFAULT_DRY_RUN_TO = "taariq@serendb.com"
STATE_TABLES = ["asset_register", "work_order_ledger", "service_history", "maintenance_calendar", "cost_actuals"]
...
def _profile(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    profile.setdefault("title", "Property & Physical-Asset Maintenance Calendar Operator")
    profile.setdefault("priority", "P1")
    profile.setdefault("cadence", "daily-due-overdue-scan-weekly-status-seasonal-templates")
    profile.setdefault("gate", "spend-committing dispatch above threshold requires approval")
    profile.setdefault("sample_subject", "Palm Beach fire inspection")
    profile.setdefault("exception_flags", ["maintenance_overdue"])
    profile.setdefault("actions", ["scan_due_maintenance", "open_work_order", "log_service_history"])
...
def _schema_guard() -> Dict[str, Any]:
    return {
        "database": "family_office_property_asset_maintenance_scheduler",
        "tables": STATE_TABLES,
