#!/usr/bin/env python3
"""Dry-run-first operator runtime for credential-hygiene-breach-monitor."""
...
SKILL_NAME = "credential-hygiene-breach-monitor"
PILLAR = "complexity-management"
DEFAULT_DRY_RUN_TO = "taariq@serendb.com"
STATE_TABLES = ["credential_inventory", "mfa_status", "breach_exposures", "remediation_tasks", "posture_history"]
...
def _profile(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    profile.setdefault("title", "Cyber Credential-Hygiene & Breach-Exposure Monitor")
    profile.setdefault("priority", "P1")
    profile.setdefault("cadence", "weekly-full-vault-audit-on-new-breach-remediation-task")
    profile.setdefault("gate", "the agent never auto-rotates passwords and logs hash references only")
    profile.setdefault("sample_subject", "Staff mailbox breach hit")
    profile.setdefault("exception_flags", ["new_breach_exposure"])
    profile.setdefault("actions", ["audit_credential_posture", "open_remediation_task", "draft_posture_report"])
...
def _schema_guard() -> Dict[str, Any]:
    return {
        "database": "family_office_credential_hygiene_breach_monitor",
        "tables": STATE_TABLES,
