---
name: credential-hygiene-breach-monitor
description: "Family office: Audits credential posture, mfa gaps, weak or reused credentials, breach exposures, remediation tasks, and posture history without storing raw passwords."
---

# Family Office · Credential Hygiene Breach Monitor

## For Claude: How to Use This Skill

Skill instructions are preloaded in context when this skill is active. Do not perform filesystem searches or tool-driven exploration to rediscover them; use the guidance below directly.

This is a recurring family-office operator skill, not a memo generator. Run it to operationalize `password-management-setup-plan` as a stateful, approval-gated employee.

## When to Use

- run credential hygiene monitor
- check breach exposure
- audit missing MFA
- review password manager posture

## Operating Contract

This operator audits credential posture, MFA gaps, weak or reused credentials, breach exposures, remediation tasks, and posture history without storing raw passwords. It belongs to `Risk, Insurance, Cyber & Physical Security` in the Citi family-office functions frame and is priority `P1` from roadmap issue #852.

Approval gate: the agent never auto-rotates passwords and logs hash references only. The agent stages review packets, audit rows, reminders, and draft communications. It does not submit money movement, government filings, binding notices, beneficiary changes, or external instructions without the live gate and human approval.

## Schema Guard

Before any read or write, resolve or create the SerenDB project `family-office-credential-hygiene-breach-monitor` and database `family_office_credential_hygiene_breach_monitor`. Verify or create these tables:
...
- `audit_credential_posture`
- `open_remediation_task`
- `draft_posture_report`

## Cloud Deployment

Deploy as a seren-cloud cron or event-triggered worker with the default Python runtime. Suggested cadence: `weekly-full-vault-audit-on-new-breach-remediation-task`. Do not use Docker, a local pull runner, or customer-specific local paths.
...
```bash
python3 -m pytest family-office/credential-hygiene-breach-monitor/tests -q
python3 family-office/credential-hygiene-breach-monitor/scripts/agent.py --functional-test --config family-office/credential-hygiene-breach-monitor/config.example.json
