#!/usr/bin/env python3
"""Dry-run-first operator runtime for tax-deadline-payment-controller."""
...
SKILL_NAME = "tax-deadline-payment-controller"
PILLAR = "complexity-management"
DEFAULT_DRY_RUN_TO = "taariq@serendb.com"
STATE_TABLES = ["tax_obligations", "payment_run_log", "safe_harbor_calculations", "voucher_packets"]
...
def _profile(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    profile.setdefault("title", "Tax Deadline & Estimated-Payment Controller")
    profile.setdefault("priority", "P0")
    profile.setdefault("cadence", "daily-90-day-horizon-quarterly-estimated-payment-surge")
    profile.setdefault("gate", "government payment submission requires --allow-live, live_mode=true, and human-witnessed approval")
    profile.setdefault("sample_subject", "Q2 estimated tax payment")
    profile.setdefault("exception_flags", ["payment_inside_action_window"])
    profile.setdefault("actions", ["recompute_tax_horizon", "draft_voucher_packet", "escalate_action_window"])
...
def _schema_guard() -> Dict[str, Any]:
    return {
        "database": "family_office_tax_deadline_payment_controller",
        "tables": STATE_TABLES,
