#!/usr/bin/env python3
"""Dry-run-first operator runtime for estate-document-vault-and-beneficiary-watch."""
...
SKILL_NAME = "estate-document-vault-and-beneficiary-watch"
PILLAR = "legacy-preservation"
DEFAULT_DRY_RUN_TO = "taariq@serendb.com"
STATE_TABLES = ["document_index", "beneficiary_designation_map", "intended_disposition", "drift_flags"]
...
def _profile(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    profile.setdefault("title", "Document-Vault Intake + Beneficiary-Designation Drift Watch")
    profile.setdefault("priority", "P1")
    profile.setdefault("cadence", "daily-vault-intake-quarterly-beneficiary-reconciliation-life-event-watch")
    profile.setdefault("gate", "the agent never changes designations and only prepares remediation packets")
    profile.setdefault("sample_subject", "IRA beneficiary designation")
    profile.setdefault("exception_flags", ["designation_drift"])
    profile.setdefault("actions", ["index_document_version", "compare_beneficiary_designation", "draft_remediation_packet"])
...
def _schema_guard() -> Dict[str, Any]:
    return {
        "database": "family_office_estate_document_vault_and_beneficiary_watch",
        "tables": STATE_TABLES,
