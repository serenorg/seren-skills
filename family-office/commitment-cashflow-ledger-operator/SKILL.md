---
name: commitment-cashflow-ledger-operator
description: "Family office: Runs the private-markets commitment ledger, capital-call intake, distribution reconciliation, wire-instruction diffing, funding reminders, and liquidity runway digest."
---

# Family Office · Commitment Cashflow Ledger Operator

## For Claude: How to Use This Skill

Skill instructions are preloaded in context when this skill is active. Do not perform filesystem searches or tool-driven exploration to rediscover them; use the guidance below directly.

This is a recurring family-office operator skill, not a memo generator. Run it to operationalize `cap-call-notification-workflow and cashflow-forecast-worksheet` as a stateful, approval-gated employee.

## When to Use

- process private markets capital calls
- run commitment cashflow ledger
- reconcile private fund distributions
- check GP wire instructions before funding

## Operating Contract

This operator runs the private-markets commitment ledger, capital-call intake, distribution reconciliation, wire-instruction diffing, funding reminders, and liquidity runway digest. It belongs to `Investment Management & Portfolio Operations` in the Citi family-office functions frame and is priority `P0` from roadmap issue #852.

Approval gate: changed GP wire instructions halt funding until out-of-band callback approval. The agent stages review packets, audit rows, reminders, and draft communications. It does not submit money movement, government filings, binding notices, beneficiary changes, or external instructions without the live gate and human approval.

## Schema Guard

Before any read or write, resolve or create the SerenDB project `family-office-commitment-cashflow-ledger-operator` and database `family_office_commitment_cashflow_ledger_operator`. Verify or create these tables:
...
- `stage_funding_plan`
- `open_wire_verification_task`
- `send_deadline_digest`

## Cloud Deployment

Deploy as a seren-cloud cron or event-triggered worker with the default Python runtime. Suggested cadence: `hourly-mailbox-portal-poll-daily-deadline-sweep-weekly-pacing-digest`. Do not use Docker, a local pull runner, or customer-specific local paths.
...
```bash
python3 -m pytest family-office/commitment-cashflow-ledger-operator/tests -q
python3 family-office/commitment-cashflow-ledger-operator/scripts/agent.py --functional-test --config family-office/commitment-cashflow-ledger-operator/config.example.json
