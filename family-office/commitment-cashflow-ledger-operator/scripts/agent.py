#!/usr/bin/env python3
"""Dry-run-first operator runtime for commitment-cashflow-ledger-operator."""
...
SKILL_NAME = "commitment-cashflow-ledger-operator"
PILLAR = "capital-allocation"
DEFAULT_DRY_RUN_TO = "taariq@serendb.com"
STATE_TABLES = ["commitment_register", "capital_call_events", "distribution_reconciliation", "funding_deadlines", "wire_instruction_history"]
...
def _profile(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    profile.setdefault("title", "Private-Markets Commitment, Capital-Call & Distribution Lifecycle Operator")
    profile.setdefault("priority", "P0")
    profile.setdefault("cadence", "hourly-mailbox-portal-poll-daily-deadline-sweep-weekly-pacing-digest")
    profile.setdefault("gate", "changed GP wire instructions halt funding until out-of-band callback approval")
    profile.setdefault("sample_subject", "Acme Growth Fund Call 7")
    profile.setdefault("exception_flags", ["wire_instruction_changed"])
    profile.setdefault("actions", ["stage_funding_plan", "open_wire_verification_task", "send_deadline_digest"])
...
def _schema_guard() -> Dict[str, Any]:
    return {
        "database": "family_office_commitment_cashflow_ledger_operator",
        "tables": STATE_TABLES,
