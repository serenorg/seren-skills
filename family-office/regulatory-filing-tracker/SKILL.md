...
description: "Family office: Tracks fbar, form 8938, 13f, 5471, 8865, 8621, 3520, and 3520-a filing thresholds and assembles review-gated packages."
---

# Family Office · Regulatory Filing Tracker

## For Claude: How to Use This Skill

Skill instructions are preloaded in context when this skill is active. Do not perform filesystem searches or tool-driven exploration to rediscover them; use the guidance below directly.

This is a recurring family-office operator skill, not a memo generator. Run it to operationalize `consolidated-reporting-spec and foreign entity inventories` as a stateful, approval-gated employee.

## When to Use

- track regulatory filings
- check FBAR and 13F thresholds
- prepare foreign information return package
- run filing threshold tracker

## Operating Contract

This operator tracks FBAR, Form 8938, 13F, 5471, 8865, 8621, 3520, and 3520-A filing thresholds and assembles review-gated packages. It belongs to `Tax & Regulatory Compliance Operations` in the Citi family-office functions frame and is priority `P1` from roadmap issue #852.

Approval gate: government e-filing remains behind dual gate and human approval. The agent stages review packets, audit rows, reminders, and draft communications. It does not submit money movement, government filings, binding notices, beneficiary changes, or external instructions without the live gate and human approval.

## Schema Guard

Before any read or write, resolve or create the SerenDB project `family-office-regulatory-filing-tracker` and database `family_office_regulatory_filing_tracker`. Verify or create these tables:

- `regulatory_filing_register`
- `threshold_check_log`
- `filing_packages`
- `foreign_account_inventory`
...
- `recompute_filing_thresholds`
- `draft_filing_package`
- `escalate_threshold_crossing`

## Cloud Deployment

Deploy as a seren-cloud cron or event-triggered worker with the default Python runtime. Suggested cadence: `quarterly-threshold-check-annual-foreign-reporting-season`. Do not use Docker, a local pull runner, or customer-specific local paths.
...
```bash
python3 -m pytest family-office/regulatory-filing-tracker/tests -q
python3 family-office/regulatory-filing-tracker/scripts/agent.py --functional-test --config family-office/regulatory-filing-tracker/config.example.json
