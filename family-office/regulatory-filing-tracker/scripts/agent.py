#!/usr/bin/env python3
"""Dry-run-first operator runtime for regulatory-filing-tracker."""
...
SKILL_NAME = "regulatory-filing-tracker"
PILLAR = "complexity-management"
DEFAULT_DRY_RUN_TO = "taariq@serendb.com"
STATE_TABLES = ["regulatory_filing_register", "threshold_check_log", "filing_packages", "foreign_account_inventory"]
...
def _profile(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    profile.setdefault("title", "Regulatory Filing Tracker (FBAR / 13F / Foreign Information Returns)")
    profile.setdefault("priority", "P1")
    profile.setdefault("cadence", "quarterly-threshold-check-annual-foreign-reporting-season")
    profile.setdefault("gate", "government e-filing remains behind dual gate and human approval")
    profile.setdefault("sample_subject", "13F threshold check")
    profile.setdefault("exception_flags", ["threshold_crossed"])
    profile.setdefault("actions", ["recompute_filing_thresholds", "draft_filing_package", "escalate_threshold_crossing"])
...
def _schema_guard() -> Dict[str, Any]:
    return {
        "database": "family_office_regulatory_filing_tracker",
        "tables": STATE_TABLES,
