#!/usr/bin/env python3
"""Dry-run-first operator runtime for multi-custodian-portfolio-aggregator-nl-qa."""
...
SKILL_NAME = 'multi-custodian-portfolio-aggregator-nl-qa'
ISSUE_NUMBER = 938
PILLAR = 'investment-operations'
DEFAULT_DRY_RUN_TO = 'taariq@serendb.com'
PROJECT_NAME = 'family-office-multi-custodian-portfolio-aggregator-nl-qa'
DATABASE_NAME = 'family_office_multi_custodian_portfolio_aggregator_nl_qa'
STATE_TABLES = ['source_file_manifest', 'unified_position_table', 'lookthrough_holdings', 'exposure_rollups', 'qa_answer_cache', 'reconciliation_break_ledger', 'stale_data_alerts']
REQUIRED_OUTPUTS = ['unified_position_table', 'lookthrough_exposure_rollup', 'nl_qa_answer_with_source_lines', 'reconciliation_break_list', 'stale_data_alerts']
HANDOFFS = ['consolidated-reporting-spec', 'consolidated-reporting-refresh-and-statement-chaser', 'cash-position-and-liquidity-monitor']
AVAILABLE_CONNECTORS = ['docreader', 'docs', 'drive', 'outlook', 'passwords', 'search', 'sheets', 'storage']
DEFAULT_PROFILE = {'title': 'Multi Custodian Portfolio Aggregator NL QA Operator', 'priority': 'P1', 'cadence': 'daily-source-refresh-weekly-reconciliation-monthly-ic-qa', 'gate': 'source-data writebacks, reconciliation adjustments, and principal distribution require human approval', 'sample_subject': 'Brazilian sovereign credit exposure through all funds', 'exception_flags': ['reconciliation_break_detected', 'stale_source_data'], 'actions': ['ingest_source_feeds', 'resolve_lookthrough_holdings', 'answer_natural_language_exposure_question', 'flag_reconciliation_breaks'], 'outputs': ['unified_position_table', 'lookthrough_exposure_rollup', 'nl_qa_answer_with_source_lines', 'reconciliation_break_list', 'stale_data_alerts'], 'handoffs': ['consolidated-reporting-spec', 'consolidated-reporting-refresh-and-statement-chaser', 'cash-position-and-liquidity-monitor'], 'always_review': True}
