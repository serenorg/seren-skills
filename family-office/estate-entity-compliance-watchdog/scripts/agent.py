#!/usr/bin/env python3
"""Dry-run-first operator runtime for estate-entity-compliance-watchdog."""
...
SKILL_NAME = "estate-entity-compliance-watchdog"
PILLAR = "legacy-preservation"
DEFAULT_DRY_RUN_TO = "taariq@serendb.com"
STATE_TABLES = ["entity_register", "annual_filings", "registered_agent_renewals", "good_standing_checks"]
...
def _profile(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    profile.setdefault("title", "Estate & Entity Annual-Filing, Registered-Agent & Good-Standing Watchdog")
    profile.setdefault("priority", "P0")
    profile.setdefault("cadence", "daily-entity-deadline-watch-weekly-good-standing-digest")
    profile.setdefault("gate", "government filings and entity changes require review and dual live gate")
    profile.setdefault("sample_subject", "Acme Holdings LLC annual report")
    profile.setdefault("exception_flags", ["annual_report_due_soon"])
    profile.setdefault("actions", ["scan_entity_deadlines", "check_good_standing", "draft_filing_packet"])
...
def _schema_guard() -> Dict[str, Any]:
    return {
        "database": "family_office_estate_entity_compliance_watchdog",
        "tables": STATE_TABLES,
