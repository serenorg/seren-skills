---
name: estate-entity-compliance-watchdog
description: "Family office: Tracks entity registers, secretary of state good standing, registered-agent renewals, annual reports, beneficial ownership changes, and compliance evidence."
---

# Family Office · Estate Entity Compliance Watchdog

## For Claude: How to Use This Skill

Skill instructions are preloaded in context when this skill is active. Do not perform filesystem searches or tool-driven exploration to rediscover them; use the guidance below directly.

This is a recurring family-office operator skill, not a memo generator. Run it to operationalize `estate plan and entity formation artifacts across the family-office catalog` as a stateful, approval-gated employee.

## When to Use

- run entity compliance watchdog
- check registered agent renewals
- monitor entity good standing
- track annual filing deadlines

## Operating Contract

This operator tracks entity registers, Secretary of State good standing, registered-agent renewals, annual reports, beneficial ownership changes, and compliance evidence. It belongs to `Estate, Trust & Entity Administration` in the Citi family-office functions frame and is priority `P0` from roadmap issue #852.

Approval gate: government filings and entity changes require review and dual live gate. The agent stages review packets, audit rows, reminders, and draft communications. It does not submit money movement, government filings, binding notices, beneficiary changes, or external instructions without the live gate and human approval.

## Schema Guard

Before any read or write, resolve or create the SerenDB project `family-office-estate-entity-compliance-watchdog` and database `family_office_estate_entity_compliance_watchdog`. Verify or create these tables:

- `entity_register`
- `annual_filings`
- `registered_agent_renewals`
- `good_standing_checks`
...
- `scan_entity_deadlines`
- `check_good_standing`
- `draft_filing_packet`

## Cloud Deployment

Deploy as a seren-cloud cron or event-triggered worker with the default Python runtime. Suggested cadence: `daily-entity-deadline-watch-weekly-good-standing-digest`. Do not use Docker, a local pull runner, or customer-specific local paths.
...
```bash
python3 -m pytest family-office/estate-entity-compliance-watchdog/tests -q
python3 family-office/estate-entity-compliance-watchdog/scripts/agent.py --functional-test --config family-office/estate-entity-compliance-watchdog/config.example.json
