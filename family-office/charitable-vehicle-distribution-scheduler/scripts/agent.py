#!/usr/bin/env python3
"""Dry-run-first operator runtime for charitable-vehicle-distribution-scheduler."""
...
SKILL_NAME = "charitable-vehicle-distribution-scheduler"
PILLAR = "legacy-preservation"
DEFAULT_DRY_RUN_TO = "taariq@serendb.com"
STATE_TABLES = ["charitable_vehicle_register", "distribution_obligations", "balance_reconciliation", "recommendation_packets"]
...
def _profile(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    profile.setdefault("title", "DAF & Charitable-Trust Distribution Scheduler & Reconciler")
    profile.setdefault("priority", "P1")
    profile.setdefault("cadence", "monthly-balance-obligation-reconcile-annual-valuation-recompute")
    profile.setdefault("gate", "DAF recommendations and trust distribution instructions require sign-off")
    profile.setdefault("sample_subject", "DAF annual giving budget")
    profile.setdefault("exception_flags", ["required_payout_due"])
    profile.setdefault("actions", ["reconcile_vehicle_balance", "compute_required_payout", "draft_distribution_packet"])
...
def _schema_guard() -> Dict[str, Any]:
    return {
        "database": "family_office_charitable_vehicle_distribution_scheduler",
        "tables": STATE_TABLES,
