#!/usr/bin/env python3
"""Dry-run-first operator runtime for cash-position-and-liquidity-monitor."""
...
SKILL_NAME = "cash-position-and-liquidity-monitor"
PILLAR = "complexity-management"
DEFAULT_DRY_RUN_TO = "taariq@serendb.com"
STATE_TABLES = ["cash_positions", "liquidity_forecasts", "cushion_breaches", "idle_cash_alerts"]
...
def _profile(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    profile.setdefault("title", "Daily Cash-Position & Liquidity-Floor Monitor")
    profile.setdefault("priority", "P0")
    profile.setdefault("cadence", "daily-balance-pull-weekly-liquidity-summary")
    profile.setdefault("gate", "monitor only; recommended sweeps or funding moves are decision packets and never money movement")
    profile.setdefault("sample_subject", "June liquidity forecast")
    profile.setdefault("exception_flags", ["projected_cushion_breach"])
    profile.setdefault("actions", ["pull_balance_snapshot", "compute_liquidity_forecast", "send_cushion_breach_alert"])
...
def _schema_guard() -> Dict[str, Any]:
    return {
        "database": "family_office_cash_position_and_liquidity_monitor",
        "tables": STATE_TABLES,
