---
name: grant-pipeline-disbursement-operator
description: "Family office: Runs grant intake, diligence, approval, scheduled disbursement, post-grant reporting, 501(c)(3) verification, sanctions screening, and report chases."
---

# Family Office · Grant Pipeline Disbursement Operator

## For Claude: How to Use This Skill

Skill instructions are preloaded in context when this skill is active. Do not perform filesystem searches or tool-driven exploration to rediscover them; use the guidance below directly.

This is a recurring family-office operator skill, not a memo generator. Run it to operationalize `family-philanthropy-strategic-plan and family-foundation-formation-plan` as a stateful, approval-gated employee.

## When to Use

- run grant pipeline
- screen grantee diligence
- stage grant disbursement for review
- chase post grant reports

## Operating Contract

This operator runs grant intake, diligence, approval, scheduled disbursement, post-grant reporting, 501(c)(3) verification, sanctions screening, and report chases. It belongs to `Philanthropy & Foundation Operations` in the Citi family-office functions frame and is priority `P0` from roadmap issue #852.

Approval gate: grant approvals and disbursements are review-gated and fail closed on sanctions hits. The agent stages review packets, audit rows, reminders, and draft communications. It does not submit money movement, government filings, binding notices, beneficiary changes, or external instructions without the live gate and human approval.

## Schema Guard

Before any read or write, resolve or create the SerenDB project `family-office-grant-pipeline-disbursement-operator` and database `family_office_grant_pipeline_disbursement_operator`. Verify or create these tables:
...
- `advance_grant_pipeline`
- `run_diligence_checklist`
- `stage_disbursement_packet`

## Cloud Deployment

Deploy as a seren-cloud cron or event-triggered worker with the default Python runtime. Suggested cadence: `daily-pipeline-advance-weekly-committee-status`. Do not use Docker, a local pull runner, or customer-specific local paths.
...
```bash
python3 -m pytest family-office/grant-pipeline-disbursement-operator/tests -q
python3 family-office/grant-pipeline-disbursement-operator/scripts/agent.py --functional-test --config family-office/grant-pipeline-disbursement-operator/config.example.json
