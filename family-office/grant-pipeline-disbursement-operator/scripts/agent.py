#!/usr/bin/env python3
"""Dry-run-first operator runtime for grant-pipeline-disbursement-operator."""
...
SKILL_NAME = "grant-pipeline-disbursement-operator"
PILLAR = "legacy-preservation"
DEFAULT_DRY_RUN_TO = "taariq@serendb.com"
STATE_TABLES = ["grant_register", "diligence_checks", "disbursement_schedule", "report_chase_tasks"]
...
def _profile(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    profile.setdefault("title", "Grant Pipeline, Diligence & Disbursement Operator")
    profile.setdefault("priority", "P0")
    profile.setdefault("cadence", "daily-pipeline-advance-weekly-committee-status")
    profile.setdefault("gate", "grant approvals and disbursements are review-gated and fail closed on sanctions hits")
    profile.setdefault("sample_subject", "Example Arts Foundation grant")
    profile.setdefault("exception_flags", ["diligence_screening_required"])
    profile.setdefault("actions", ["advance_grant_pipeline", "run_diligence_checklist", "stage_disbursement_packet"])
...
def _schema_guard() -> Dict[str, Any]:
    return {
        "database": "family_office_grant_pipeline_disbursement_operator",
        "tables": STATE_TABLES,
