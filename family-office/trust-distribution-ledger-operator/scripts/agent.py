#!/usr/bin/env python3
"""Dry-run-first operator runtime for trust-distribution-ledger-operator."""
...
SKILL_NAME = "trust-distribution-ledger-operator"
PILLAR = "legacy-preservation"
DEFAULT_DRY_RUN_TO = "taariq@serendb.com"
STATE_TABLES = ["distribution_schedule", "distribution_ledger", "beneficiary_wire_instructions", "trustee_approvals"]
...
def _profile(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    profile.setdefault("title", "Trust Distribution Scheduling & Execution Operator")
    profile.setdefault("priority", "P0")
    profile.setdefault("cadence", "daily-due-distribution-tick-quarterly-annual-fixed-distributions")
    profile.setdefault("gate", "release requires dual live gate and human-witnessed trustee approval")
    profile.setdefault("sample_subject", "GRAT annuity distribution")
    profile.setdefault("exception_flags", ["beneficiary_wire_mismatch"])
    profile.setdefault("actions", ["compute_distribution_amount", "open_trustee_approval", "reconcile_distribution_debit"])
...
def _schema_guard() -> Dict[str, Any]:
    return {
        "database": "family_office_trust_distribution_ledger_operator",
        "tables": STATE_TABLES,
