#!/usr/bin/env python3
"""Dry-run-first operator runtime for travel-security-keyperson-watch."""
...
SKILL_NAME = "travel-security-keyperson-watch"
PILLAR = "complexity-management"
DEFAULT_DRY_RUN_TO = "taariq@serendb.com"
STATE_TABLES = ["trip_register", "threat_indicators", "travel_alerts", "provider_roster", "keyperson_register"]
...
def _profile(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    profile.setdefault("title", "Travel-Security & Threat-Intelligence Watch Operator")
    profile.setdefault("priority", "P1")
    profile.setdefault("cadence", "daily-feed-calendar-correlation-pretrip-t7-t2-briefs")
    profile.setdefault("gate", "the agent never cancels travel or dispatches protection without approval")
    profile.setdefault("sample_subject", "London family office trip")
    profile.setdefault("exception_flags", ["corridor_risk_threshold_crossed"])
    profile.setdefault("actions", ["correlate_trip_threats", "draft_pretrip_brief", "open_travel_alert"])
...
def _schema_guard() -> Dict[str, Any]:
    return {
        "database": "family_office_travel_security_keyperson_watch",
        "tables": STATE_TABLES,
